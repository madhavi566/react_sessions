...
const RestrauntCard = ({
  name,cloudinaryImageId,cuisines,avgRating,user
}) => {
  const {beginner} = useContext(UserContext);

  return (
    <div className="space-x-2 hover:bg-purple-300 w-60">
      <img className="w-[200]" src={IMG_CDN_URL + cloudinaryImageId} />
      <h2>{name}</h2>
      <h3>{cuisines.join(", ")}</h3>
      <h4>{avgRating} minutes</h4>
      <h3> {user.name}</h3>
      <p> User context date</p> 
      <span className="text-red-800 underline">{beginner.name} - {beginner.email}</span> 
    </div>
  );
};
